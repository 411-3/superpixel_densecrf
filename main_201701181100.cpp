...
// 21 color for labels
int colors[21] = {	0, 128, 32768, 32896, 8388608, 8388736,8421376, 8421504, \
					64, 192, 32832, 32960,8388672, 8388800, 8421440, 8421568, \
					8192, 16512, 49152, 49280, 8404992 \
				 };
...
int main(int argc, char* argv[])
{
	...
	cnpy::NpyArray arr = cnpy::npy_load(img_fcn);	
   	float * raw_unary  = (float *)malloc(sizeof(float) * w * h * CLASS);
   	float * unary      = (float *)malloc(sizeof(float) * w * h * CLASS);
	float temp_score(0);
	...
	for(i=0; i<w*h; i++)
	{
		...
		for(j=0; j<CLASS; j++)
		{
			if( raw_unary[ i * CLASS + j ] > max_pro )
			{
				max_pro = raw_unary[ i * CLASS + j ];
				map[i] = j;
			}
		}
	}
	printf("\n");
	for(i=w*h/2-200; i<w*h/2-190; i++)	printf("%d-",map[i]);
	printf("\n");
